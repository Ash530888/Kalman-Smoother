...
def main(argv):
    ...
    Gamma=Sigma=np.array([[100,100],[100,100]])

    w=np.transpose(np.random.multivariate_normal([0,0],Gamma,number_positions))
    v=np.transpose(np.random.multivariate_normal([0,0],Sigma,number_positions))
    ...
    return filterRes["xnn"], smoothRes["xnN"]
